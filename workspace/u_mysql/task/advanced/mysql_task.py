...
#
# parent_id = save(insert_query, parent_datas).get('seq')
# print(parent_id)
#
# # 아이가 있다면 아이의 정보까지 입력
...
# if parent_id:
# # 로그인된 학부모의 아이 정보 출력 - 여기부터 수정필요
#     login_data_query = "SELECT id, child_name, child_age, child_gender, parent_id \
#     FROM tbl_child"
#     rows = find_all(login_data_query)
#     for row in rows:
#         print(row)
# # 아이의 정보 중 나이를 수정하기(아이 고유 번호를 이용)
#     while True:
#         choice = input("아이 나이를 변경하시겠습니까?[y/N]\n")
#         if choice == 'N':
#             break
#
#         child_update_query = "UPDATE tbl_child SET child_age=%s \
#         WHERE child_id=%s"
#         update_child_age = input("아이 나이: ")
#         update_child_id = input("아이 고유 번호: ")
#         update(update_child_age, update_child_id)


# if parent_id:
#     # 로그인된 학부모의 아이 정보 출력
#     login_data_query = "SELECT id, child_name, child_age, child_gender, parent_id FROM tbl_child"
#     rows = find_all(login_data_query)
#     for row in rows:
#         print(f'아이 정보 -> {row}')
#
...
#         choice = input("\n아이 정보 수정 menu\n1.아이 나이 변경\n""2.아이 정보 추가\n""3.체험학습 지원하기\n""9.나가기\n")
#         if choice == "1":
#             child_update_query = "UPDATE tbl_child SET child_age=%s WHERE id=%s"
#             update_child_id = input("아이 고유 번호: ")
#             update_child_age = input("아이 나이: ")
#             update(child_update_query, (update_child_age, update_child_id))
#             continue
#
# # 새로운 아이 정보 추가
#         elif choice == "2":
...
#             child_age = input("아이 나이: ")
#             child_gender = input("아이 성별: ")
#             child_datas = [child_name, child_age, child_gender, parent_id.get('id')]
#             save(insert_query, child_datas)
#             continue

# 체험학습지원
#         elif choice == "3":
#             trip_data_query = "SELECT id, field_trip_title, field_trip_content, field_trip_number FROM tbl_field_trip"
#             results = find_all(trip_data_query)
#             for result in results:
#                 print(f'✔️체험학습\n{result}')
#
#             insert_query = "INSERT INTO tbl_apply (child_id, field_trip_id) \
#             VALUES(%s, %s)"
#             child_id = input("아이 고유 번호: ")
#             field_trip_id = input("체험학습 번호: ")
#
...
# parent_name = input("관리자 명: ")
# parent_phone = input("관리자 코드: ")
#
...
#
# for i in range(30):
#     index = random.randint(0, get_count_product(get_count_product_query).get('product_count') - 1)
