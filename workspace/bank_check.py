def check(*, key, value): # 키와 벨류값을 받아서 유져에게 리턴
    def set_target():# set_target으로 Bank.banks -> bank -> user 전달
...
class Bank: # 뱅크 클래스
    ...
    banks = [[] for i in range(total_count)] # 리스트 안에 리스트 세개를 담기 위한 코드


# __init__메소드 ->생성자/ 멤버의 변수를 초기화. __init__의 호출이 끝나면 메모리 주소값이 부여
# self 객체 자기 자신을 참조하는 변수, self가 아닌 다른 변수명을 사용할 수 있다. 멤버 메소드정의 시 사용.
    ...
    def open_account(cls, owner: str, account_number: str, phone: str, password: str, money: int, bank_choice: int): # 
        # bank_choice - 1 을 하는 이유는 인덱스 값으로 찾기때문
        bank = [
